...
def analysis(client,users,methods):
  ...
  for key in users.keys():
    ...
    report=analysis_methods_params(key,client,methods,ignored_methods)
    ...
    if not method in ignored_methods:
      try:
        ...
        if not re.search(r'\(\)',method) and key in t2:
          ...
          #Code to generate all unique parameter-datatype combinations; I'll comment this out once I use it once :)
   #       t3=t1.group(2).split(',')
   #       while k<len(t3):
   #         t3[k]=re.sub(r'^\s+',r'',t3[k])
   #         t3[k]=re.sub(r'\s+$',r'',t3[k])
   #         if t3[k] not in datatype:
   #           datatype[t3[k]]=''
   #         k+=1
          #End of Code to generate all unique parameter-datatype combinations. Print it out outside the loop to generate all valid datatypes
