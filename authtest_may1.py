...
def main():
  ...
  #What methods do you currently want to test? This list needs to keep getting updated as your requirements change
  test_methods=get_methods_to_test()

  #Login as each potential adversary and query each method.
  report=analysis(client,users,methods,test_methods)

  #Create report after analysis is complete. The results are written into a CSV file.
...
def get_methods_to_test():
  test_methods={}
  try:
    f=open('methods_to_test','rU')
  except:
    print 'Could not open file containing methods to be tested'
...
def analysis(client,users,methods,test_methods):
  ...
  ignored_methods = ['GetAllGuidanceItems()','GitHub_Push_Origin()','Logout()','GetAllUserLogs()','GetDeletedLibraries()','GitHub_Push_Commit()','DeleteDeletedGuidanceTypes()','DeleteDeletedLibraries()','GitHub_Pull_Origin()','GetGuidanceTypes()','GetGuidanceTypes()','GetGuidanceTypes()']
  ...
  for key in users.keys():
    ...
    analysis_methods_no_params(key,client,methods,ignored_methods,test_methods)

    #Analyze methods that have one or more parameters. For e.g XmlDatabase_GetGuidanceItemXml(ns0:guid guidanceItemId, )
    report=analysis_methods_params(key,client,methods,ignored_methods,test_methods)
...
def analysis_methods_no_params(key,client,methods,ignored_methods,test_methods):
  for method in methods.keys():
    t4=method.split('(')
    if not method in ignored_methods and method in test_methods:
      try:
        ...
        if re.search(r'\(\)',method) and key in t2:
          ...
          t3=re.search(method_correct_value[t4[0]],str(b),re.DOTALL)
          if t3:
            report[key+'^'+method+'^'+'Unsafe\n']=''
          else:
            report[key+'^'+method+'^'+'Output format incorrect. Please check this manually.\n']=''
        elif re.search(r'\(\)',method) and key not in t2:
          report[key+'^'+method+'^'+'NA\n']=''
      except Exception:
...
def analysis_methods_params(key,client,methods,ignored_methods,test_methods):
  for method in methods.keys():
    t4=method.split('(')
    i=0;l1='';t1='';t2='';k=0
    if not method in ignored_methods and method in test_methods:
      try:
        ...
        if not re.search(r'\(\)',method) and key in t2:
          ...
          if t3:
            ...
            report[key+'^'+method+'^'+'Output format incorrect. Please check this manually.\n']=''
        elif not re.search(r'\(\)',method) and key not in t2:
          report[key+'^'+method+'^'+'NA\n']=''
      except Exception:
        #Code reaches here only if eval fails; we want to track why..hence sys.exc_info()
        exc_type, exc_value = sys.exc_info()[:2]
